const MISSING_KEY_RE = /GROQ_API_KEY is missing|not configured in environment/i;
...
export function formatGeminiErrorForUi(geminiError) {
  ...
  if (MISSING_KEY_RE.test(s)) {
    return 'The API server does not have GROQ_API_KEY set. Add it to the `.env` file next to `index.js`, restart the server, then run a new assessment.';
  }
...
export function isPlaceholderExplanation(text) {
  return Boolean(text && /Personalized AI text could not be loaded|check GROQ_API_KEY/i.test(String(text)));
}
